...
2. 프로젝트의 **SQL Editor**를 열고 `supabase/schema.sql` 파일의 전체 내용을
   ...
   수정할 수 있게 하는 컬럼, `003_extra_stages.sql`과 `004_unified_stages.sql`
   — 증강체가 등장하는 스테이지를 사람이 직접 체크박스로 관리할 수 있게 하는
   컬럼들 — 004가 003을 대체하지만 003도 반드시 먼저 실행해야 004의 데이터
   이전 로직이 정상 동작합니다). 이후 이 프로젝트를 업데이트하면서 새
...
## 9. 어떤 증강체가 어느 스테이지에 등장하는지 직접 관리하기

Community Dragon 데이터에는 "이 증강체가 2-1/3-2/4-2 중 정확히 어디서
등장하는지"에 대한 정보가 없습니다. 이건 metatft나 롤체지지 같은 외부
사이트를 매번 안정적으로 긁어오기엔(구조가 언제 바뀔지 모르고, 이번
프로젝트에서 CDragon 파싱만도 여러 라운드가 걸렸습니다) 신뢰성이 떨어질 것
같아 **사람이 직접 체크하는 방식**으로 만들었습니다.

- 증강체는 등급(실버/골드/프리즘)과 무관하게, **기본적으로 어느 스테이지에도
  체크되어 있지 않은 상태**로 시작합니다(자동 배정 없음). `npm run
  fetch:augments` / "패치 데이터 새로고침"은 이름·설명·아이콘·등급만 갱신할
  뿐, 어느 스테이지에 등장하는지는 절대 건드리지 않습니다.
- 각 스테이지 페이지 상단의 **"+ 증강체 추가"**를 펼치면 전체 증강체를
  이름으로 검색할 수 있습니다. 아직 이 스테이지에 없는 증강체를 찾아서
  "추가"를 누르면 바로 이 스테이지의 "미분류" 목록에 나타납니다.
- 증강체 상세 모달을 열면 맨 위에 "등장 스테이지" 체크박스 3개(2-1/3-2/4-2)가
  있고, 셋 다 자유롭게 켜고 끌 수 있습니다(셋 다 꺼진 상태도 유효 — 아직
  확인 안 된 증강체라는 뜻). 지금 보고 있는 스테이지의 체크를 끄면 그 보드
  에서 바로 사라집니다. 켜둔 스테이지에서는 서로 독립적으로 티어를 매길 수
  있습니다(같은 증강체도 스테이지마다 다른 티어 가능).
- 즉 실제 플레이하면서 "이 증강체 3-2에서도 나오네" 싶을 때마다 그 자리에서
  체크해서 데이터를 조금씩 채워나가는 방식입니다.
